...
class DriverNavViewController: UIViewController, MKMapViewDelegate, CLLocationManagerDelegate {
    ...
    @IBAction func confirmButton(_ sender: UIButton) {
        confirm = true
        let value = ["riderName": myRiderName, "riderLat": myRiderLat, "riderLong": myRiderLong, "driverLat": driverLocation?.latitude, "driverLong": driverLocation?.longitude, "driverArrived": "false", "driverPhone": phone, "driverName": name, "carColor": carColor, "carYear": carYear, "carMake": carMake, "carModel": carModel, "carPlate": carLicencePlate] as [String : Any]
        let rideReference = self.ref?.child("acceptedRides").child(myRiderID)
        rideReference?.updateChildValues(value)
